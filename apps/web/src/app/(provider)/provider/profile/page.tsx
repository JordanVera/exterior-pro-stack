...
export default function ProviderProfilePage() {
  ...
  useEffect(() => {
    Promise.all([trpc.provider.getProfile.query(), trpc.service.list.query()])
      .then(([p, s]) => {
        setProfile(p); setAllServices(s); setBusinessName(p.businessName); setDescription(p.description || ""); setServiceArea(p.serviceArea || "");
        const selected = new Map<string, number | undefined>();
        p.services.forEach((ps: any) => { selected.set(ps.service.id, ps.customPrice ? Number(ps.customPrice) : undefined); });
  ...
  const handleSaveProfile = async () => {
    setSaving(true);
    try { await trpc.provider.updateProfile.mutate({ businessName, description: description || undefined, serviceArea: serviceArea || undefined }); alert("Profile updated!"); }
  ...
  const handleSaveServices = async () => {
    ...
    try {
      ...
      await trpc.provider.setServices.mutate({ services }); alert("Services updated!");
    } catch (err: any) { alert(err.message || "Failed to update services"); }
    finally { setSaving(false); }
  };
  ...
  return (
    <div className="space-y-8 max-w-3xl">
      ...
      <div className="bg-white dark:bg-neutral-900 rounded-xl p-6 border border-gray-200 dark:border-neutral-800 space-y-4">
        <h3 className="font-semibold text-gray-900 dark:text-white">Business Information</h3>
        <div><label className={labelClass}>Business Name *</label><input type="text" value={businessName} onChange={(e) => setBusinessName(e.target.value)} className={inputClass} /></div>
        <div><label className={labelClass}>Description</label><textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} className={`${inputClass} resize-none`} /></div>
        <div><label className={labelClass}>Service Area</label><input type="text" value={serviceArea} onChange={(e) => setServiceArea(e.target.value)} className={inputClass} placeholder="e.g., Dallas-Fort Worth metro" /></div>
        <button onClick={handleSaveProfile} disabled={saving} className="px-6 py-2 bg-green-600 text-white font-medium rounded-lg hover:bg-green-700 disabled:opacity-50 transition-colors">
      ...
      <div className="bg-white dark:bg-neutral-900 rounded-xl p-6 border border-gray-200 dark:border-neutral-800 space-y-4">
            ...
            return (
              <div key={service.id} className={`flex items-center gap-3 p-3 rounded-lg border transition-colors ${
                ...
                <div className="flex-1">
                  ...
                  <span className="text-xs text-gray-500 dark:text-neutral-400 ml-2">({service.category.name})</span>
                </div>
                ...
                {isSelected && (
                  <input type="number" step="0.01" min="0" placeholder="Custom $"
                    ...
                    className="w-24 rounded border border-gray-300 dark:border-neutral-700 px-2 py-1 text-sm bg-white dark:bg-neutral-950 text-gray-900 dark:text-white"
                  />
                )}
              </div>
            );
        ...
        <button onClick={handleSaveServices} disabled={saving} className="px-6 py-2 bg-green-600 text-white font-medium rounded-lg hover:bg-green-700 disabled:opacity-50 transition-colors">
